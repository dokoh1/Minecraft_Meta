...
오브젝트의 곡면 역시 작은 삼각형들의 집합으로 근사되며, GPU가 한 프레임을 그리기 위해서는 다음 정보가 필요하다.
...
2차원 공간에서 사각형(Quad) 모델을 만든다고 하면 네 개의 정점(Vertices)을 지정해야 한다.
...
모든 3D 모델의 면은 삼각형(Triangle)들의 조합으로 구성된다. 사각형(Quad)은 두 개의 삼각형으로 만들 수 있다.
...
Unity의 Rendering 및 대부분의 엔진 API는 메인 스레드에서만 접근할 수 있기 때문에
...
이 프로젝트는 이 프로젝트에서는 '계산은 백그라운드 스레드, Mesh 적용은 정확한 PlayerLoop 타이밍의 메인 스레드'라는 구조가 필수적이었다.

여기에 수명/취소 안전과 낮은 GC, 간결한 코드까지 요구되었다.
...
본 프로젝트에서 사용한 방식 외에도 다양한 최신 기법들이 존재하므로, 더 폭넓게 리서치하여 상황에 가장 적합한 접근법을 선택하는 것이 중요하다.
